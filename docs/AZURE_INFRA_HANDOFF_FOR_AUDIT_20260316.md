...
Important note:
- `/docs`, `/openapi.json`, and `/redoc` are currently auth-exempt in code, in [route_deps.py](/Users/pedromousinho/Downloads/dbde_ai_assistant-main/route_deps.py)
- this matters for audit because it exposes the API shape publicly unless separately blocked at edge or ingress
...
Important note:
- the publish settings file contains deployment credentials and should be treated as sensitive material
- if this file was shared externally, rotate publish credentials
...
## Portal Checks an Auditor Should Ask Someone With Access to Confirm
These checks do not require Azure CLI if done in the Azure Portal.

### App Service
1. Open `millennium-ai-assistant`
2. Confirm:
   - runtime stack
   - startup command
   - Always On
...
### Worker app
1. Open `millennium-ai-assistant-worker`
2. Confirm:
   - app exists and is running
   - startup command
   - health check path
   - app settings like `WORKER_MODE`
...
### Storage account
1. Open `dbdeaccessstorage`
2. Confirm:
   - region
   - networking exposure
   - public access disabled for containers where appropriate
   - lifecycle management
   - table list and container list

### Search service
1. Open `dbdeacessrag`
2. Confirm:
   - which indexes actually exist
   - document counts for legacy vs story indexes
   - auth model and key rotation

### Key Vault
1. Open `dbde-ai-vault`
2. Confirm:
   - access policies or RBAC
   - managed identities with access
   - which secrets are stored there vs still sitting in app settings

### Monitor
1. Confirm action group `dbde-ai-alerts`
2. Confirm the four expected metric alerts
3. Confirm whether App Insights is used or not

## Likely Resource Inventory
Based on repo intent, the Azure estate likely includes at least:
...
- 1 AI Search service
- 1 Key Vault
- 1 Automation Account or scheduled automation footprint
- 1 autoscale setting
- 1 action group
- multiple Azure Monitor metric alerts
- 1 or more Azure AI services for OpenAI/Language/Content Safety/Document Intelligence/Speech
